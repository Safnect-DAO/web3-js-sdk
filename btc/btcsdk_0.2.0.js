...
*/
export async function getAddr(network, publicKeyX, publicKeyY, addressType) {
  ...
  let param = {
     publicKey: pub.toHex(),
     //addressType: "Legacy", // mw6QyHdAr1wJL9vJxYWeotXWKD2tNjKbej
...
/* 获取钱包余额(单位是Satoshi，转换成BTC需要除以1亿）
  参数：
    network 网络，取值mainnet|testnet
    address  钱包地址
    callback(success, data)  回调，余额=（chain_stats.funded_txo_sum - chain_stats.spent_txo_sum）/ 100000000
...
*/
export async function getBalance(network, address, callback) {
  var url;
  if (network == mainnet) {
    url = mainnet_rpc_url_prefix; // mainnet
  } else {
    url = testnet_rpc_url_prefix; // testnet
  }
  url += 'address/' + address;
...
*/
export async function getBillList(network, address, callback) {
  var url;
  if (network == mainnet) {
    url = mainnet_rpc_url_prefix; // mainnet
  } else {
...
*/
export async function getGasFeeList(network, callback) {
...
export async function getUtxos(network, address, callback) {
  var url;
  if (network == mainnet) {
    url = mainnet_rpc_url_prefix; // mainnet
  } else {
...
export function getPrivateKeyWIF(network, privateKeyStr) {
...
*/
export async function buildTx(network, senderAddr, amount, privateKeyStr, receiverAddr, feePerB, callback) {
  getUtxos(network, senderAddr, function(success, re) {
    re.sort((a, b) => b.value - a.value);
    var arr = [];
    var totalSt = 0;
    re.forEach((utxo) => {
      if (totalSt - amount < 10000) {
        arr.push({ txId: utxo.txid, vOut: utxo.vout, amount: utxo.value });
        totalSt += utxo.value;
      }
    });
...
*/
export async function estimateGasFee(network, signParams, callback) {
  var wallet;
  if (network == mainnet) {
    wallet = new BtcWallet(); // mainnet
  } else {
...
*/
export async function sendTx(network, signParams, callback) {

  var wallet;
  var broadcastUrl;
  if (network == mainnet) {
    wallet = new BtcWallet(); // mainnet
    broadcastUrl = mainnet_rpc_url_prefix + 'tx' // mainnet
  } else {
    wallet = new TBtcWallet(); // testnet
    broadcastUrl = testnet_rpc_url_prefix + 'tx'; // testnet
  }
...
获取比特币实时价格（USD）
  ...
  data: 
    {"bitcoin":{"usd":69399,"usd_24h_change":-2.8389067324642947}}
     bitcoin.usd 实时价格（USD），bitcoin.usd_24h_change 24小时涨跌幅（%）
*/
export async function getBTCPrice(coin, callback) {
